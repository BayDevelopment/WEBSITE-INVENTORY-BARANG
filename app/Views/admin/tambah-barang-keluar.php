          ...
          <form action="<?= base_url('admin/data-barang-keluar/tambah') ?>" method="post" class="needs-validation" novalidate>
            ...
            <div class="form-floating mb-3">
              ...
              <label for="tanggal_keluar">
                <i class="fa-solid fa-calendar-days me-2 text-primary"></i>Tanggal Keluar
              </label>

              <div class="invalid-feedback">
                <?= $validation->getError('tanggal_keluar') ?: 'Tanggal keluar wajib diisi' ?>
              </div>
            </div>

            <!-- Status -->
            <?php $selectedStatus = old('status') ?: ($currentStatus ?? ''); ?>
            <div class="form-floating mb-3">
              <select name="status"
                      id="status"
                ...
                <option value="" disabled <?= empty($selectedStatus) ? 'selected' : '' ?>>
                  -- Pilih Status --
                </option>

                <?php foreach (['menunggu','disetujui','ditolak'] as $status): ?>
...
<script>
  ...
  function toggleAlasanPenolakan() {
    ...
    if (status === 'ditolak') {
      ...
      // optional: clear value ketika bukan ditolak
      // alasan.value = '';
    }
  }
