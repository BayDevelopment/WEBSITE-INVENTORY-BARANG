...
<style>
  ...
  /* Responsive scanner box: tidak terlalu tinggi, mengikuti lebar */
  #scanner {
    width: 100%;
    border-radius: 14px;
    overflow: hidden;
    background: #0b1220;
    position: relative;
    aspect-ratio: 16 / 9;      /* responsif */
    max-height: 360px;         /* biar tidak tinggi */
    min-height: 220px;         /* tetap usable */
  }
  ...
  #scanner canvas {
    ...
    object-fit: cover;         /* penting: video memenuhi tanpa jadi tinggi */
  }
  ...
  /* Kotak panduan scan, ikut responsif */
  .scan-box {
    width: min(82%, 380px);
...
<div class="container-fluid px-4">
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
    <div>
      <h2 class="fw-bold text-primary mb-1">Scan Barang Masuk</h2>
...
<script>
  ...
  async function handleFound(rawCode) {
    ...
    try {
      ...
      // pastikan response JSON (biar ketahuan kalau kena redirect/login)
      ...
      const target = `<?= base_url('admin/data-barang-masuk/tambah') ?>?id_barang=${encodeURIComponent(json.data.id_barang)}`;
  ...
  function startScan() {
    ...
    Quagga.init({
      inputStream: {
        ...
        // biar "kena semua" dan tidak terlalu sempit:
